...
if (reuse == 0)
    ...
    % based on previous run of BayOpt
    fprintf('*** Reusing previous run of BayOpt ***\n')
    load(['BayOpt_ALL_HV',HV,'.mat'],'ResLVVol','ResLVStrain','x','y','Times','reuse_number')
    reuse_number = reuse_number + 1;
    fprintf('*** reuse_number = %i ***\n', reuse_number)
end
...
[gp_rec,gp_mcmc,opt_mcmc] = gp_mc(gp, x, y, 'nsamples', 220,'display',20);
...
%     y(end+1,:) = fx(x_new);     % calculate the function value at query point
    ...
    save(['BayOpt_ALL_HV',HV,'_',num2str(reuse_number),'_moreEV.mat'],'x','y','ResLVVol','ResLVStrain','i1','Times','gp','lik','cfse','controls','reuse_number')
end 
